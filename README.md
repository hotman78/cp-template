...
本テンプレートは以下に依存しておりますが、後述コマンドで全部入るはずです (cargo は AHC にしか使わないのでとりあえず除外しました)
...
インストールするのは以下のコマンドで可能です(コマンド は vscode 上で `Ctrl+j` を入力することで出てくる場所に書き込みます)
`(今いる場所)/(後に指定するフォルダ名)` に実行環境が整備されます
...
コマンドはターミナルから実行します。 vscode 上で `Ctrl+j` を入力することでターミナルを出せます。

## ログイン

AtCoder の仕様変更によって面倒になりました...
https://github.com/key-moon/aclogin
に沿ってログインすれば良いです
